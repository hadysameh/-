  ...
  return (
    <>
      <div className="row">
                ...
                <ul style={{ listStyle: "none" }}>
                  {Array.isArray(selectedBranches) &&
                    ...
                    selectedBranches.map((branch: any) => {
                      return (
                        <li className="text-secondary" key={branch.name}>
                          {branch.name}
                        </li>
                      );
                    })}
        ...
        {Array.isArray(branchesChoises) && !isArrEmpty(branchesChoises) && (
          <div className="col-6">
            <label className="form-label" id="branches-select-label">
              {" "}
              تعديل الأفرع المختصة{" "}
            </label>
