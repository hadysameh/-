...
export default class SaderBoxRepo {
  ...
  public static async getWithParams(
    ...
    console.log({
      unreadSader: searchParams.unreadSader,
    });
    if (searchParams.unreadSader == "true") {
      whereParams["doc_date"] = {
        [Op.gte]: `${config?.getDataValue("dateOfLaunch")}`,
      };
      let sadersThatOfficerSaw = await Sader.findAll({
        where: {
          doc_date: {
            [Op.gte]: `${config?.getDataValue("dateOfLaunch")}`,
          },
      ...
      let sadersIdsThatOfficerSaw = sadersThatOfficerSaw.map(
        (sader: any) => sader.id
      );
     
       
      whereParams["id"] = {
        [Op.notIn]: sadersIdsThatOfficerSaw,
      };
    }
